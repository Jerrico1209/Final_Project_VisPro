...
namespace prototypeapp
{
    public partial class Poin : Form
    {
        ...
        private void TxtUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                query = string.Format("UPDATE `poin` SET `Username`='{0}', `NomorKamar`='{1}', `Midweek`='{2}', `Vesper`='{3}', `Sabbath` = '{4}', `PA` = '{5}' where `Username` = '{6}'", TxtNama.Text, TxtNomorKamar.Text, TxtMidweek.Text, TxtVesper.Text, TxtSabbath.Text, TxtPA.Text, TxtNama.Text);
                ds.Clear();
                koneksi.Open();
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
